...
-- Insert live locations for drivers (online status) - using 'rider' for user_type and 'bike' for motorcycle vehicle_type
INSERT INTO live_locations (user_id, user_type, latitude, longitude, status, vehicle_type, accuracy, speed, heading)
SELECT 
    user_id,
    'driver',  -- Using 'driver' for user_type
    0.3476 + (random() - 0.5) * 0.01,
    32.5825 + (random() - 0.5) * 0.01,
    'online',
    'car',     -- Using 'car' for vehicle_type
    8.5,
    0,
    90
FROM profiles WHERE email = 'sarah.passenger@zoto.com'
...
SELECT 
    user_id,
    'rider',   -- Using 'rider' for boda-boda user_type
    0.2937 + (random() - 0.5) * 0.01,
    32.6147 + (random() - 0.5) * 0.01,
    'online',
    'bike',    -- Using 'bike' for motorcycle vehicle_type
    6.2,
    15,
    180
FROM profiles WHERE email = 'mirujeremy@gmail.com';
